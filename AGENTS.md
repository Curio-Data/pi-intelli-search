...
- **`Pi` >= 0.81.1:** Core functionality, trusted project settings, `CONFIG_DIR_NAME`, provider-based `pi-ai` calls, and async model-registry refresh. `Pi` >= 0.86 dispatches LLM calls through the `ctx.modelRegistry.streamSimple()` facade (required on those versions: providers drop a `systemPrompt` handed to them directly); `Pi` 0.81.1-0.85.x use the direct provider path. Compatibility audited and verified through `Pi` 0.87.0 (2026-09-22; see CHANGELOG). The `fetch`/`onPayload` request hooks used by annotation harvesting and the web search tool are verified against pi-ai 0.87.0.
